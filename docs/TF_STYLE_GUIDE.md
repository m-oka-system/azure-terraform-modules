...
モジュールの主リソース（ファイル先頭）にのみ 3 行見出しを付けます。サブリソースには見出しを付けません。補足が必要な場合のみ 1 行コメントを付けます。

```hcl
# Good: 主リソースに 3 行見出し、サブリソースは見出しなし
################################
# Storage Account
################################
resource "azurerm_storage_account" "this" {
...
# Defender for Storage（補足が必要な場合のみ 1 行コメント）
resource "azurerm_security_center_storage_defender" "this" {
  # ...
}
```
...
属性名から意味が自明な場合、コメントは不要です。属性名を日本語に言い換えるだけのコメントは冗長なため書きません。
...
# Good: コメントなしで記述（属性名が自明）
default_outbound_access_enabled   = each.value.default_outbound_access_enabled
private_endpoint_network_policies = each.value.private_endpoint_network_policies

# Good: 属性名だけでは意図がわからない設定にコメントを付ける
vnet_image_pull_enabled = true # VNet 統合を経由してコンテナーイメージをプル
```
