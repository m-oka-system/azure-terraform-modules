...
- **コミットメッセージは日本語で:** 全ての git コミットメッセージは Conventional Commits 形式（`feat`, `fix`, `refactor`, `docs`, `chore`, `test`, `style`）で日本語で書く。Claude Code の署名（Co-Authored-By）は含めない

## 技術リファレンス

- @README.md - ディレクトリ構成、セットアップ手順、コマンド一覧、テスト方法
- @.claude/rules/terraform.md - Terraform 開発ルール・検証コマンド
- @docs/TF_STYLE_GUIDE.md - HCL コードスタイルガイド
- `.claude/skills/terraform-*/` - スキル定義（リサーチ・実装・検証・ワークフロー）
...
- ユーザーからの修正指摘があった場合は必ず: `claudedocs/lessons.md` にパターンを記録する（ファイルが存在しない場合は新規作成する）
...
1. **計画を先に**: `claudedocs/todo.md` にチェック可能な項目付きの計画を書く（ファイルが存在しない場合は新規作成する）
