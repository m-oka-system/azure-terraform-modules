...
Agent が失敗した場合（エラー、タイムアウト、空の結果）は、以下の手順で対応してください:

1. どの Agent が失敗したかをユーザーに報告する
2. 成功した Agent の結果で統合を続行する
3. 失敗した Agent が担当するセクションには「未取得 - {Agent名} が応答なし（{エラー内容}）」と記載する
4. 3 つすべてが失敗した場合は、処理を中断し「全 Agent の調査に失敗しました。MCP サーバーの状態を確認してください」と通知する
