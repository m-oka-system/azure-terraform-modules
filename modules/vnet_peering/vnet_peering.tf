resource "azurerm_virtual_network_peering" "hub_to_spoke" {
  ...
  allow_forwarded_traffic      = true  # Hub が Spoke から転送されてきたトラフィック（他の Spoke や オンプレミスからのトラフィック）を受け入れるか（Spoke 間通信が必要な場合は true）
...
resource "azurerm_virtual_network_peering" "spoke_to_hub" {
  ...
  allow_virtual_network_access = true
  allow_forwarded_traffic      = false # Spoke が他のネットワークから転送されてきたトラフィックを受け入れるか（通常 Spoke では false）
